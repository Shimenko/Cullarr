...
RSpec.describe Sync::RunSync, type: :service do
  ...
  it "executes all sync phases and persists phase counts" do
    ...
    library_mapping_phase = instance_double(Sync::TautulliLibraryMappingSync, call: { "rows_processed" => 5 })
    ...
    allow(Sync::TautulliLibraryMappingSync).to receive(:new).and_return(library_mapping_phase)
    ...
    expect(result.keys).to eq(
      %w[sonarr_inventory radarr_inventory tautulli_users tautulli_library_mapping tautulli_history tautulli_metadata mapping_risk_detection cleanup]
    )
    expect(sync_run.reload.phase_counts_json).to include(
      "tautulli_library_mapping" => { "rows_processed" => 5 },
